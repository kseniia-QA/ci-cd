...
movies.forEach(movie => {
   ...
   cy.get('input[name="name"]').type(movie.movie,{force: true})
   cy.get('input[type="text"]').eq(1).type('200',{force: true}) 

   cy.get('textarea.conf-step__input').type('BestMovie', {force: true})
   cy.get('input[type="text"]').eq(2).type('Russia', {force: true})
  cy.get('input[value="Добавить фильм"]').click()

  //    cy.get('input[value="Загрузить постер"]').click().attachFile('cypress\fixtures\ILTQq.png');
})
})
})
...
it('Checks user can go to a new movie session', () => {
      ...
      cy.get('a.movie-seances__time').eq(3).click()
